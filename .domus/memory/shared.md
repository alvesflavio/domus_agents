...
- Generated Codex agent TOML files include `display_name = "<specialist_name>"` as an experimental Codex Desktop Subagents UI hint while preserving the technical `name` for routing.
- Shared memory uses a low-token stack: read `.domus/memory/state.md` and `.domus/memory/inbox.md` first, then `shared.md`, agent-specific memory, and only read `handoffs.md` when compact memory is insufficient or history is requested. `workstyle-standards-coordinator` owns stack initialization and task queue updates when invoked for delegation or shared memory.
